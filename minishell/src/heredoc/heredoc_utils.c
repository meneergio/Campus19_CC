...
/*   Updated: 2025/12/01 16:07:32 by gwindey          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	prepare_heredocs_one_cmd(t_redir *r, t_env_entry *env, int last_status)
{
	while (r)
	{
		if (r->type == R_HEREDOC)
		{
			if (!handle_one_heredoc(r, env, last_status))
				return (0);
		}
		r = r->next;
	}
