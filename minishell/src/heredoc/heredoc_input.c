...
/*   Updated: 2025/12/01 16:17:46 by gwindey          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	heredoc_write_one(const char *line, int fd, t_heredoc_ctx *ctx)
{
	char	*expanded;
	...
	expanded = expand_word_all(line, ctx->env, ctx->last_status);
	if (!expanded)
	{
		write(fd, "\n", 1);
		return ;
					...
					t_heredoc_ctx *ctx)
{
	...
	while (1)
	{
		...
		heredoc_write_one(line, hdoc_fd_write, ctx);
		free(line);
	}
	close(hdoc_fd_write);
	return (0);
