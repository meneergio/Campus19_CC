...
/*   Updated: 2025/12/01 16:19:25 by gwindey          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	heredoc_child_process(int fds0, int fds1, t_redir *r,
							t_heredoc_ctx *ctx)
{
	close(fds0);
	...
	read_heredoc_input(r->arg, fds1, ctx);
	close(fds1);
	exit(0);
}
...
int	handle_one_heredoc(t_redir *r, t_env_entry *env, int last_status)
{
	...
	if (pid == 0)
		heredoc_child_process(fds[0], fds[1], r, &ctx);
	close(fds[1]);
	return (heredoc_wait_and_cleanup(pid, fds[0], r, &last_status));
}
...
int	prepare_heredocs(t_ast *ast, t_env_entry *env, int last_status)
{
	...
	while (i < ast->ncmd)
	{
		if (!prepare_heredocs_one_cmd(ast->cmdv[i].redirs, env, last_status))
		{
